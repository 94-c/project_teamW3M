					...
					<div class="mlog-sign">
							...
							<form action="/login.do" name="form1" method="post">
								...
								<p class="se-log">
									<label><input type="checkbox" name="ssl" value="Y"
										class="MS_security_checkbox" checked /> 보안접속</label>
								</p>
								<c:if test="${msg eq 'failure' }">
									<p style="color: red; font-size: 14px" align="right">아이디 또는
										비밀번호가 일치하지 않습니다. 다시 입력해주세요.</p>
								</c:if>
								<c:if test="${msg eq 'suspended' }">
									<p style="color: red; font-size: 14px" align="center">정지된
										계정입니다.</p>
								</c:if>
								<c:if test="${msg eq 'delete' }">
									<p style="color: red; font-size: 14px" align="center">탈퇴한
										계정입니다.</p>
								</c:if>
						...
						<div class="sign">
							...
							<dl>
								...
								<dd>
									<a href="findIDForm.do" class="CSSbuttonWhite fe">아이디/비밀번호
										찾기</a>
								</dd>
							</dl>
						</div>
				...
				<div id="simpleJoinSns">
					<div class="sns-area">
						<a href="https://kauth.kakao.com/oauth/authorize?client_id=cd6fd139e4c5a14b7e6272d000e94c30&redirect_uri=http://localhost:8080/kakaoCallback.do&response_type=code"
							class="link kakao">
							<img src="resources/images/sns/kakao_join.png" alt="카카오톡으로 로그인">
						</a>
						<a href="${url}" class="link naver">
							<img src="resources/images/sns/naver_join.png" alt="네이버로 로그인">
						</a>
					</div>
				</div>
			</div>
